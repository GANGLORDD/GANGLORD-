...
function setupQuickView() {
    ...
    if (quickViewBtns.length === 0) {
        console.warn('⚠️ No se encontraron botones de vista rápida');
        return;
    }
...
const products = {
    ...
    4: {
        ...
        name: "Turnal",
        price: 130000,
        image: "assets/turnalfrente.jpeg",
        description: "Edición limitada con acabados premium"
    },
    5: {
        id: 5,
        name: "Shadow Lord Tee",
        price: 160000,
        image: "assets/shadowfrente.jpeg",
        description: "Nueva colección exclusiva 2025"
    },
    6: {
        id: 6,
        name: "Look At Me",
        price: 130000,
        image: "assets/lookfrente.jpeg",
    ...
    7: {
        ...
        name: "Essencial Gold",
        price: 170000,
        image: "assets/goldfrent.jpeg",
        description: "Edición nocturna exclusiva"
    },
    8: {
        id: 8,
        name: "Essential Blue Dark",
        price: 100000,
        image: "assets/ebfrente.jpeg",
        description: "Colección esencial en azul oscuro"
    },
    9: {
        id: 9,
        name: "Essencial Blue",
        price: 100000,
        image: "assets/ebcfrente.jpeg",
        description: "Colección esencial en azul"
    },
    10: {
        id: 10,
        name: "Essencial White",
        price: 80000,
        image: "assets/ewfrente.jpeg",
        description: "Colección esencial en blanco"
    }
};
...
function changeToImage(productCard, imageIndex) {
    ...
    if (!images || !images[imageIndex]) {
        console.error('❌ No se encontró la imagen en el índice:', imageIndex);
        return;
    }
    ...
    if (!currentImg) {
        console.error('❌ No se encontró la imagen actual');
        return;
    }
    
    console.log('🔄 Cambiando de:', currentImg.src, 'a:', images[imageIndex]);
    
    // Cambiar imagen con efecto de transición
    currentImg.style.opacity = '0';
    
    setTimeout(() => {
        // Verificar que la imagen existe antes de cambiarla
        const newImg = new Image();
        newImg.onload = function() {
            currentImg.src = images[imageIndex];
            currentImg.style.opacity = '1';
            console.log('✅ Imagen cargada correctamente:', images[imageIndex]);
        };
        newImg.onerror = function() {
            console.error('❌ Error al cargar la imagen:', images[imageIndex]);
            currentImg.style.opacity = '1'; // Restaurar opacidad aunque falle
        };
...
// Función para el showcase del nuevo lanzamiento
let currentShowcaseIndex = 0;
const showcaseImages = ['assets/shadowfrente.jpeg', 'assets/shadowatras.jpeg'];

function changeShowcaseImage(direction) {
    console.log('🔄 Cambiando imagen del showcase:', direction);
    
    currentShowcaseIndex += direction;
    
    if (currentShowcaseIndex < 0) {
        currentShowcaseIndex = showcaseImages.length - 1;
    } else if (currentShowcaseIndex >= showcaseImages.length) {
        currentShowcaseIndex = 0;
    }
    
    const showcaseImg = document.getElementById('showcaseImg');
    const indicators = document.querySelectorAll('.image-indicators-showcase .indicator');
    ...
    if (showcaseImg) {
        ...
        setTimeout(() => {
            const newImg = new Image();
            newImg.onload = function() {
                showcaseImg.src = showcaseImages[currentShowcaseIndex];
                showcaseImg.style.opacity = '1';
                console.log('✅ Imagen del showcase cargada:', showcaseImages[currentShowcaseIndex]);
            };
            newImg.onerror = function() {
                console.error('❌ Error al cargar imagen del showcase:', showcaseImages[currentShowcaseIndex]);
                showcaseImg.style.opacity = '1';
            };
            newImg.src = showcaseImages[currentShowcaseIndex];
...
function showProductModal(productCard) {
    ...
    try {
        ...
        document.getElementById('modalProductDescription').textContent = productDescription || 'Producto de alta calidad con diseño exclusivo';
